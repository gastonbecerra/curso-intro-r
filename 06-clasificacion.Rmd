...
En este capítulo nos introduciremos a una técnica de aprendizaje supervisado, que busca clasificar texto en base a otros documentos ya clasificados. 

Particularmente, vamos a trabajar con un corpus de oraciones que tratan con el big data, que han sido catalogadas manualmente como positivas o negativas. En este sentido, este ejercicio continúa, con otro método, el análisis de sentimiento que vimos en un capítulo anterior.

A lo largo de este ejercicio veremos:

1. pre-procesamiento de texto;
2. vectorización de los textos;
3. subdivisión del corpus para entrenamiento y testeo;
...
library(stringr) # para modificar cadenas de palabras
library(caret) # para modelo
library(tm) # para modelo

oraciones <- readr::read_csv("./data/oraciones_entrenar.csv") # importamos los datos
glimpse(oraciones) # exploramos la estructura de los datos
table(oraciones$target) # vemos la distribución de las clasificaciones
...
Comúnmente, los modelos de *machine learning* son entrenados con datos estructurados en forma de tablas. Cuando trabajamos con texto debemos construir estas tablas a partir de las palabras de la oración, o del documento con el que estemos trabajando. Esto lo hacemos con el *vectorizado*.

Supongamos que tenemos dos frases: "El big data es el conjunto de técnicas que las grandes corporaciones analizan para manipular nuestro pensamiento en función de sus intereses privados” y "Google analiza big data para inferir el ritmo de contagio de la gripe H1N1", que en su forma lemmatizada y filtrada serían "bigdata ser conjunto tecnica grande corporacion analizar manipular pensamiento funcion interes privado" y "google analizar bigdata inferir ritmo contagio gripe h1n1".
...
Con R es muy fácil generar estos vectores y manipularlos.
