...
La validación de hipótesis con contenido y metadatos (también llamada validación semántica intratópico y externa, respectivamente) consiste en hipotetizar cierta información presente en nuestros datos para explorar desde nuestro modelo. 

Por ejemplo, en nuestra base de `noticias` tenemos algunos meta-datos (como la fecha de publicación de los artículos) con información que nuestro modelo no analizó, de modo que podríamos usarlos para evaluar su coherencia. Para ello, pensemos en 
...
Para ello, debemos volver a seleccionar las noticias más representativas de un tópico y 
...
noticias_tm_gamma %>%
  ...
  group_by(nombre, month = lubridate::floor_date(fecha, unit = "month")) %>%
  summarize(n=n()) %>%
